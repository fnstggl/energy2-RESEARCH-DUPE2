#!/usr/bin/env python3
"""Register the bounded-ingested Exgentic/agent-llm-traces config into the
canonical corpus registry.

Reads the per-config summary.json under
data/external/hf/Exgentic__agent-llm-traces/<config>/processed/ and
re-writes data/external/hf_discovery/canonical_corpus_registry.json
via aurelius.traces.hf_corpus.promotion.

Audit-only. No production claim.
"""
