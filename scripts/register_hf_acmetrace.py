#!/usr/bin/env python3
"""Register the 4 AcmeTrace configs (Qinghao/AcmeTrace) into the canonical
corpus registry.

Reads each per-config summary.json under
``data/external/hf/Qinghao__AcmeTrace/<config>/processed/`` and writes the
union (existing entries + new entries) back to
...
def main() -> int:
    ...
    for dataset_id, config in NEW_ENTRIES:
        ...
        appended += 1
        print(
            f"  registered {dataset_id}@{config} "
