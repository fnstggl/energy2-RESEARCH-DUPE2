...
**Research Audit: PARD Admission-Control + BurstGPT Ceiling Analysis [run 2026-06-25 session 2] — USEFUL RESEARCH (Five-Failure Rule compliant):**
Merged PR #82 (research docs, 0% KPI drift). Left PRs #83 and #81 open (architecture-unification requires human review). All 5 canonical benchmarks stable: AMCSG Azure 150,630 / BurstGPT 168,270 / OSOTSS Azure +5.94% / OSOTSS BurstGPT +5.85% / energy +11.33% (all match ROADMAP, 0.00% KPI drift on serving). 92 parity tests pass. BurstGPT ceiling analysis: conformal arrival-rate would recover at most +0.26% n_sla_safe (15 requests), yielding estimated +0.3-0.5% additional goodput/$. Low priority. 5 new papers reviewed: (1) "Beyond Prediction: Tail-Aware Scheduling" (ICML 2026) NOT APPLICABLE (per-request intra-instance scheduler); (2) "TIE: Uncertainty-Aware Output Length Predictions" NOT DIRECTLY APPLICABLE (per-request SJF); (3) "Hermes: Probabilistic Demand Modeling" NOT APPLICABLE (multi-step app pipelines); (4) **"PARD: Proactive Request Dropping" (EuroSys 2026) — PRIORITY CANDIDATE** (directly maps to Aurelius's AdmissionPolicy stub, +16-176% goodput on 64-GPU cluster, uses only decision-time remaining-latency-budget telemetry, blocked by Five-Failure Rule + PR #83 pending human review); (5) "Large-Scale LLM Inference: Asymptotically Optimal Control" THEORETICALLY RELEVANT (validates Erlang-C gate-and-route as asymptotically optimal structure). Priority ranking for post-rule experiments: (1) PARD admission control, (2) Phase 1b-C energy overlay on serving, (3) conformal arrival-rate.
