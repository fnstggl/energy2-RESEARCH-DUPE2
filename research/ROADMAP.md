...
**Phase 1b-A: ReplayHarness unified entry point [run 2026-06-25] — ARCHITECTURE CONVERGENCE (Phase 1b-A complete, Five-Failure Rule compliant):**
Implements `ReplayHarness` class + `ReplayHarnessConfig` dataclass in `aurelius/optimizer/replay_harness.py` (~230 LOC). Creates the single dispatch layer that routes any Aurelius replay benchmark through a unified `run()` API and returns `ReplayEvaluationResult` objects (Phase 1b-B schema). Supports all four benchmark IDs: `replica_scaling` (full dispatch to `backtest.run_backtest()`), `genai_serving` (full dispatch to `genai_backtest.run_backtest()`), `energy` (dispatch to `canonical_backtests.run_canonical_backtest()`), `serving_queue` (pass-through adapter from pre-computed sim_dicts). KPI identity verified on BurstGPT (51 req) and Azure LLM 2024 fixtures — bit-identical to direct backend calls. 22 new Phase 1b-A parity tests (all pass). Total: 153 canonical parity tests (22+131). KPI change: 0.00%. Five-Failure-Rule compliant: no new module, no new optimizer path, no new experiment. Canonical replays confirmed: AMCSG Azure 150,630 / BurstGPT 168,270 ✓; Energy canonical 0.337299 gp/$ (+11.1%) ✓.
Results: `research/results/phase1b_a_replay_harness_2026-06-25.md`
Tests: `tests/test_phase1b_a_replay_harness_parity.py` (22 tests, all pass).
