...
**BurstGPT-as-BE Closed-Loop Compounding Validation [run 2026-06-25] — NULL RESULT on compounding robustness (USEFUL RESEARCH, Five-Failure Rule compliant, 7/5 ACTIVE):**
PR hygiene: No new open PRs to classify. Canonical replays: control arm (A) jobs_hash=82a6768ff5f35025 matches prior PR #87 result (C+O+A=75,224 gp/$ +9.00%, C=73,643 +6.71%, COMPOUNDING) — bit-identical, 0% drift. New experiment (B): Azure LC + BurstGPT-distribution BE (mean=340 tok vs Azure's 116 tok, 3× heavier). Finding: **compounding does NOT persist with heavier BE tokens**. With BurstGPT-BE: best single=O (+4.76%), best multi=O+A (+1.16%) — interaction=SUBSTITUTIVE, margin=−3.44% (multi BELOW single). Capacity lever (C) becomes counterproductive (−15.23% vs base) because 3× heavier BE jobs drive up GPU costs beyond goodput gain (c_mean 4.53→7.17, cost $10.87→$17.20). SRPT ordering alone is dominant: longer differential between short LC and long BE jobs makes SRPT more effective at protecting LC SLAs at zero marginal cost. Compounding result from PR #87 is distribution-dependent: requires BE-to-LC token ratio near 1×; at 3× ratio, ordering (O) is the dominant single lever and combining levers is substitutive. Same-conditions: same LC trace, same SLA, same on-demand denominator, same physics, same evaluation method; only BE token pool changed. Five-Failure counter: UNCHANGED (replay validation work).
Results: `research/results/burstgpt_be_closed_loop_2026-06-27.{md,json}`.
Script: `scripts/run_burstgpt_be_closed_loop.py`.

**Post-Phase-1b Research Audit [run 2026-06-25] — USEFUL RESEARCH (Five-Failure Rule compliant, 6/5 ACTIVE):**
PR hygiene: PR #81 (Phase 1b-A) classified as architecture unification → needs human review, not merged. PRs #70 and #54 left open (non-main base / dirty respectively). 4 canonical replays confirmed (186 parity tests pass, 0% KPI drift). 5 papers reviewed: (1) arXiv:2303.17551 (SIGMETRICS 2023 Pause-Resume) — THEORETICAL FOUNDATION: proves O(log T) regret bound for pure temporal shifting; our +11.1% energy gain is near the achievable limit from this lever alone. (2) arXiv:2508.16449 (GreenLLM DVFS 2025) — NOT APPLICABLE: requires per-GPU power telemetry absent from public benchmarks. (3) arXiv:2408.07831 (CarbonClipper ST-CLIP, SIGMETRICS 2025) — NOT APPLICABLE: requires multi-datacenter spatial dimension. (4) arXiv:2508.14544 (Adaptive Robust Conformal 2025) — PARTIALLY APPLICABLE: arrival-rate conformal bounds for OSOTSS (different from prior token-length use). Blocked by Five-Failure Rule. Priority 1 post-Five-Failure experiment. (5) arXiv:2503.07545 (Queueing+LLMs) — PARTIALLY APPLICABLE: historical-variance margin for arrival-rate replica scaling (prior review was for token-length SRTF, different domain). Priority 2. Architecture gap: Phase 1b-A (PR #81) still needs human review. Five-Failure counter: UNCHANGED.
