...
+26.05%) and land at ~88% retention. The ML prior is genuinely active (n_model_ids=2, CV
15.3%→43.0%, MAE −2.5%) but its marginal accuracy gain never reaches the SLA threshold;
residual error is the same irreducible ChatGPT intra-class variance ceiling from runs -u/-v/-w.
α barely moves between priors (abs: 0.000562 global vs 0.000595 ML). **Structural implication:
investing in better causal output-length predictors will not move SLA-safe goodput/$ on
BurstGPT — the abs-conformal calibrator already extracts the available scheduling signal
prior-agnostically. The remaining north-star lever is compound economic × queue scheduling
(run -y Q2/Q6), not prediction accuracy.** Harness cross-validates exactly against the three
merged results (global+rel=34,003.6=run -t; global+abs=42,901.6=run -x; ml+rel=33,962.3=run -v).
17 new tests (13 unit + 4 integration). Results:
...
**Prior quality:** ML is genuinely active (n_model_ids=2, CV 15.3%→43.0%, MAE 166.9→162.8,
−2.5%). It is sharper-but-more-volatile; abs mean α barely moves (0.000562 global vs
0.000595 ML).
...
3. **Investing in better causal output-length predictors will not move SLA-safe goodput/$
   ...
   (run -y Q2/Q6), not prediction accuracy.

**Harness validation:** reproduces the three merged results exactly — global+rel=34,003.6
