...
| **Phase 3d — Route GenAI constraint_aware policy through AureliusOptimizer + GenAIServingPolicy** | **DONE — 0% KPI drift, bit-identical replica counts, physics moved to canonical owner** | New `aurelius/optimizer/policies/genai_serving.py` owns `genai_effective_service_s`, `genai_eval_tick_timeout`, `genai_size_for_sla`, `genai_size_for_target` + constants. `genai_backtest.py` delegates local physics wrappers to the canonical module and routes `constraint_aware` through `_GENAI_OPTIMIZER.optimize(ticks, cold)`. `POLICY_REGISTRY["genai_serving"] = GenAIServingPolicy`. 6 parity tests (`tests/test_genai_canonical_routing_parity.py`): 6 pass. Existing 7 ablation tests still pass. `research/results/genai_canonical_routing_phase3d_2026-06-24.md` |
