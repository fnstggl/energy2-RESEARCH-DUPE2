...
| **Phase 4 — Causal adaptive rho via frontier estimator in constraint_aware** | **PARTIAL — implemented, null result on fixtures (MIN_REPLICAS floor); retained for production-scale evaluation** | `compute_frontier_rho_schedule()` added to `replica_scaling.py`; `constraint_aware_adaptive` policy added to `backtest.py`; `ReplicaScalingConfig.adaptive_frontier_window` field added. BurstGPT + Azure LLM 2024 fixtures: +0.00% (load too low for rho differences to cross integer replica thresholds). `research/results/phase4_frontier_rho_results.json`. |
