...
| **Phase 1b-A — ReplayHarness unified dispatch layer (serving + replica-scaling + genai + energy)** | **DONE — 0% KPI drift, 22 parity tests, all 4 backends dispatched** | `aurelius/optimizer/replay_harness.py` (~230 LOC): `ReplayHarness` class + `ReplayHarnessConfig` + `ReplayHarnessError`; exported from `aurelius.optimizer`; `tests/test_phase1b_a_replay_harness_parity.py` (22); `research/results/phase1b_a_replay_harness_2026-06-25.md`. KPI identity verified on BurstGPT (51 req) and Azure LLM 2024 fixtures — bit-identical to direct backend calls. |
