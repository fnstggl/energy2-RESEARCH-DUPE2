...
| **Phase 1b-A — Unified replay ENGINE (one closed discrete-event loop, all serving surfaces on one evolving state)** | **DONE — closed joint loop; measured COMPOUNDING (+9.00%) on a multi-class trace vs NEUTRAL on single-class, same code** | `aurelius/optimizer/unified_replay.py` (single event-heap loop: arrivals+completions+tick control; pluggable Capacity/Admission controllers; capacity REACTS to live backlog; class-aware multi-tier; on-demand denominator; `run_unified_combination` scored by `ObjectiveLayer`). `AureliusOptimizer.optimize_joint_closed_loop()`. Canonical multi-class dataset: `aurelius/datasets/{signal_matrix,canonical}.py`. `scripts/run_unified_replay_ab.py`; `tests/test_unified_replay.py` (6) + `test_canonical_dataset.py` (5); `research/results/unified_replay_compounding_ab_2026-06-25.md` + `research/CANONICAL_PRODUCTION_DATASET_DESIGN.md`. |
