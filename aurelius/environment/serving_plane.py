...
class ServingPlane:
    ...
    def build_requests(
        ...
        kv: KVReuseModel | None = None, idx_offset: int = 0,
    ) -> list:
        ...
        fleet's priority mix); KV hits are marked by ``kv``.
        """
        ...
        for i, (arr, tok) in enumerate(slice_sorted):
            ...
            out.append(ServingRequest(
                ...
                kv_prefix_id=("hit" if hit else ""),
                kv_reuse_prob=(1.0 if hit else 0.0)))
        return out

    def run_hour(
        self, requests: list, fleet, *, tick_seconds: float, sla_s: float,
        kv: KVReuseModel | None = None,
        ...
        capacity; KV hits discount service time (prefill savings). No M/M/1.
        """
        kv = kv or KVReuseModel()
        jobs = [
            Job(idx=r.idx, arrival_s=r.arrival_s, actual_tokens=r.tokens,
                predicted_tokens=r.predicted_tokens,
                service_s=kv.service_s(r.tokens, bool(r.kv_prefix_id)), cls=r.cls)
            for r in requests
        ]
        ...
        action = {
            ...
            "warmup_c": warmup_c, "kv_hit_rate": kv.hit_rate,
            "n_kv_hits": sum(1 for r in requests if r.kv_prefix_id),
