...
def classify_scenario(name: str,
    ...
    """
    # Telemetry-failsafe detection by name.
    lname = name.lower()
    is_failsafe = any(
    ...
    # Explicit-override path: explicit keys win over rule-based inference.
...
def select_headline_baseline(metadata: ScenarioMetadata,
    ...
    Rules (in order):
      ...
      3. inference_critical / inference_standard: prefer 'sla_aware' if safe;
         else 'fifo'.
      4. batch_inference / batch_training / embedding_offline / fine_tuning:
         pick the strongest of the relevant_baselines by
         sla_safe_goodput_per_infra_dollar, EXCLUDING any candidate whose
         sla_violations > fifo's OR p99 > 1.5x fifo's.
         If all candidates fail safety, return
         ('fifo', 'headline_baseline_disqualified_for_safety').
      5. fragmentation_packing intent: pick the best packing baseline if
         policy_results contains one with a goodput/$ value, else the
         strongest safe non-packing candidate, else fifo.
      6. otherwise: strongest safe non-oracle candidate, else fifo.
    """
    if metadata is None:
    ...
    # Rule 3 — interactive workloads prefer SLA-aware if safe
    ...
    # Rule 5 — fragmentation_packing picks best packing baseline by goodput/$
    if metadata.optimization_intent == "fragmentation_packing":
        ...
        for n in packing_names:
            ...
            if v is not None and v > best_val:
                best, best_val = n, v
        if best is not None and best_val > -math.inf:
            return (best, "best_packing_baseline_by_goodput_per_dollar")
        # Fall through to rule 4 logic on non-packing candidates.

    # Rule 4 / 6 — strongest safe non-oracle candidate from relevant_baselines
    best = None
    best_val = -math.inf
    any_disqualified = False
    for name in metadata.relevant_baselines:
        if name == "clairvoyant_lower_bound":  # oracle, not deployable
            continue
...
def analyze_outcome(metadata: ScenarioMetadata,
                    ...
                    *, scorecard_flags=()) -> OutcomeAnalysis:
